...
;; A Dir is (list Str FDList)
;; * where first is a name
;; * second is an FDList
...
;; A FileDir is (anyof File Dir) of:
;; * a File
;; * a Dir
  
;; A File is a (list Str Nat Nat)
;; * where first is a name
;; * second is a file size
;; * third is a timestamp

;; make and accessor functions for File:
  (define (make-file name size timestamp)
    (list name size timestamp))
  ...
  (define (file-timestamp file)
    (third file))

  (define (file? item)
    (and (cons? item) (cons? (rest item)) (cons? (rest (rest item))) (empty? (rest (rest (rest item))))
         (string? (first item))
         (and (integer? (second item)) (> (second item) 0))
         (and (integer? (third item)) (> (third item) 0))))

;; make and accessor functions for Dir:
  (define (make-dir name fdl)
    (list name fdl))
  ...
  (define (dir? item)
    (and (cons? item) (cons? (rest item)) (empty? (rest (rest item)))
         (string? (first item))
         (filedir? (second item))))
  
(define (filedir? item)
  (cond [(empty? item) true]
        [(cons? item) (and (or (file? (first item)) (dir? (first item))) (filedir? (rest item)))]
        [else false]))
  ...
  (define sample-fs
    (make-dir "root"
              (list 
               ...
               (make-dir "photos"
                         (list 
                          (make-file "marks.jpg" 3669000 1319216638)
                          (make-file "timbit.jpg" 2866000 1319181806)
                          (make-file "luna.jpg" 2709000 1319287654)
                          (make-dir "vacation"
                                    (list                    
  ...
  ;; lofd-print: (listof (anyof File Dir)) (listof boolean) -> String
  ;; Purpose: Recurse through a directory, dispatching each entry
